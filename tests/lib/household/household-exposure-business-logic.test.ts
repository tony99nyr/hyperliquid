...
describe('computeHouseholdExposure', () => {
  it('values collateral balances at mark and picks the dominant leg', () => {
    const e = computeHouseholdExposure({ weEth: 7, nativeEth: 0, wBtc: 0.01, usdc: 500 }, marks);
  ...
  it('counts NATIVE ETH in the ETH delta (post leverage-retirement the Safe holds raw ETH)', () => {
    const e = computeHouseholdExposure({ weEth: 1.4, nativeEth: 8, wBtc: 0, usdc: 0 }, marks);
    expect(e.ethExposureUsd).toBe((1.4 + 8) * 2000);
    expect(e.dominant).toBe('ETH');
  });

  it('BTC-dominant when wBtc value exceeds weETH value', () => {
    const e = computeHouseholdExposure({ weEth: 0.1, nativeEth: 0, wBtc: 0.5, usdc: 0 }, marks);
    expect(e.dominant).toBe('BTC');
  });

  it('dominant none when both legs are dust', () => {
    const e = computeHouseholdExposure({ weEth: 0, nativeEth: 0, wBtc: 0, usdc: 1000 }, marks);
    expect(e.dominant).toBe('none');
    expect(e.netCryptoBetaUsd).toBe(0);
  });

  it('never negative or NaN on garbage balances / zero / NaN marks', () => {
    const e = computeHouseholdExposure({ weEth: -5, nativeEth: -1, wBtc: NaN, usdc: -10 }, { ethUsd: 0, btcUsd: NaN });
...
describe('stackingUsd', () => {
  const exposure = computeHouseholdExposure({ weEth: 7, nativeEth: 0, wBtc: 0, usdc: 0 }, marks); // $14k long ETH
