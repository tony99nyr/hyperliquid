#!/bin/sh
# Stop the trade-watch service. Sends SIGTERM so the loop finishes its in-flight
# cycle and exits cleanly (the script traps SIGTERM). -f/--force escalates to sudo.
...
if [ $stopped -eq 0 ]; then
    pids=$(ps -ef 2>/dev/null | grep "[s]cripts/trader-watch.ts" | awk '{print $2}')
    for pid in $pids; do
