#!/bin/sh
# Cron-friendly watchdog: ensure the trade-watch service is running; restart it if
...
# Load .env (for HEALTHCHECKS_TRADER_WATCH_URL) if present.
...
if [ -n "$HEALTHCHECKS_TRADER_WATCH_URL" ] && command -v curl >/dev/null 2>&1; then
    if [ "$HAD_ERROR" -eq 1 ]; then
        curl -fsS -m 10 --retry 3 "${HEALTHCHECKS_TRADER_WATCH_URL}/fail" >/dev/null 2>&1
    else
        curl -fsS -m 10 --retry 3 "$HEALTHCHECKS_TRADER_WATCH_URL" >/dev/null 2>&1
    fi
fi
